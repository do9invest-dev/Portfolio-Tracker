"""
Portfolio Dashboard — Equal-Weight & Target-Weight Rebalancing Tracker
"""
...
PORTFOLIO_FILLS = [
    ...
    "rgba(246,135,179,0.12)",
    "rgba(79,209,197,0.12)",
]
# Red is reserved for the benchmark so it never collides with a portfolio line.
BENCH_COLOR = "#fc8181"
BENCH_FILL = "rgba(252,129,129,0.10)"

# Max portfolios that can be overlaid at once — bounded by the palette so each
# series always gets a distinct color.
MAX_PORTFOLIOS = len(PORTFOLIO_COLORS)
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        ...
        help=(
            "Columns: Ticker, Date, and optionally Weight. Each unique date "
            "is a rebalance date. If a Weight column is present (e.g. "
            "'1.00%' or 0.01), those target weights are used; otherwise "
            "tickers in that date are equal-weighted."
        ),
    )
    if uploaded:
        for f in uploaded:
    ...
    # Portfolio selection — up to MAX_PORTFOLIOS
    portfolios = list_portfolios()
    if not portfolios:
        st.info("Upload a CSV to get started.")
        st.stop()

    st.markdown(f"#### Select Portfolios (up to {MAX_PORTFOLIOS})")
    selected = st.multiselect(
        "Portfolios",
        portfolios,
        default=portfolios[:1],
        max_selections=MAX_PORTFOLIOS,
    ...
    for name in selected:
        ...
        for i, (date, holdings) in enumerate(schedule):
            ...
            with st.expander(f"{label} — {date.strftime('%Y-%m-%d')}", expanded=(i == len(schedule) - 1)):
                for t, w in sorted(holdings, key=lambda x: -x[1]):
                    st.markdown(f"` {t} ` — {w:.1%}")
...
for idx, name in enumerate(port_data):
    ...
    for date, _holdings in port_data[name]["schedule"]:
...
with st.expander("📋 Full Rebalance Schedule", expanded=False):
    for name in port_data:
        st.markdown(f"**{name}**")
        rows = []
        for date, holdings in port_data[name]["schedule"]:
            for t, w in sorted(holdings, key=lambda x: -x[1]):
                rows.append({
